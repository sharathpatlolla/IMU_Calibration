...
ay=zeros(1,Limit-1);
az=zeros(1,Limit-1);
AccelerationMatrix=zeros(12,Limit-1);
...
% If the required number of values are obtained from the orientation, it tells you to change the orientation.
        ...
        if SwitchingVariable(1,i)<7
            AccelerationMatrix(SwitchingVariable(1,i),i)=new(3)*LocalGravity;
        else
            AccelerationMatrix(SwitchingVariable(1,i),i)=new(4)*LocalGravity;
            AccelerationMatrix(SwitchingVariable(1,i)+1,i)=new(5)*LocalGravity;
            AccelerationMatrix(SwitchingVariable(1,i)+2,i)=new(6)*LocalGravity;
        end
        ax(1,i)=new(4);
        ay(1,i)=new(5);
        az(1,i)=new(6);
...
EqualInclinationPositiveG_ax=sum(AccelerationMatrix(7,:))/RowSearch(AccelerationMatrix,7);
EqualInclinationPositiveG_ay=sum(AccelerationMatrix(8,:))/RowSearch(AccelerationMatrix,8);
EqualInclinationPositiveG_az=sum(AccelerationMatrix(9,:))/RowSearch(AccelerationMatrix,9);
EqualInclinationNegativeG_ax=sum(AccelerationMatrix(10,:))/RowSearch(AccelerationMatrix,10);
EqualInclinationNegativeG_ay=sum(AccelerationMatrix(11,:))/RowSearch(AccelerationMatrix,11);
EqualInclinationNegativeG_az=sum(AccelerationMatrix(12,:))/RowSearch(AccelerationMatrix,12);

BiasMatrix=[PositiveG_ax+NegativeG_ax;PositiveG_ay+NegativeG_ay;PositiveG_az+NegativeG_az]/2
ScalarFactorMatrix=[PositiveG_ax-NegativeG_ax 0 0;0 PositiveG_ay-NegativeG_ay 0;0 0 PositiveG_az-NegativeG_az]/(2*LocalGravity)
MisalignmentMatrix=[0 EqualInclinationPositiveG_ay+EqualInclinationNegativeG_ay EqualInclinationPositiveG_az+EqualInclinationNegativeG_az;EqualInclinationPositiveG_ax+EqualInclinationNegativeG_ax 0 EqualInclinationPositiveG_az+EqualInclinationNegativeG_az;EqualInclinationPositiveG_ax+EqualInclinationNegativeG_ax EqualInclinationPositiveG_ay+EqualInclinationNegativeG_ay 0]/(2*LocalGravity) 
